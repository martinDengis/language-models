...
def save_training_plots(exp_dir, train_losses, test_losses, perplexities, sample_generations=None):
    ...
    try:
        ...
        plt.savefig(os.path.join(exp_dir, "training_plots.pdf"), format='pdf', bbox_inches='tight')
        plt.close()
...
exp_dir = create_experiment_dir(base_dir="experiments", sub_folder="exp2", prefix="exp2")
...
def run_experiments(train_loader, test_loader, vocab, device, config):
    ...
    # Save hyperparameters to the experiment directory
    ...
    for name, model in models.items():
        model_path = os.path.join(model_results_path, f'model_{name}.pt')
