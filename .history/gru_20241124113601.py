...
# Search for training_results.json in all subdirectories
lstm_results_files = sorted(
    glob.glob(os.path.join(lstm_results_dir, "**", "training_results.json"), recursive=True),
    reverse=True
)
...
# Use the latest LSTM results file
latest_lstm_results_path = lstm_results_files[0]

# Extract directory name for hyperparameter matching
lstm_dir_name = os.path.basename(os.path.dirname(latest_lstm_results_path))
match = re.search(r"lstm_h(\d+)_l(\d+)", lstm_dir_name)
if match:
    hidden_size = int(match.group(1))
    num_layers = int(match.group(2))
else:
    raise ValueError("Could not extract hyperparameters from LSTM directory name")

# Debug: Check what was loaded
print(f"Using LSTM results file: {latest_lstm_results_path}")
print(f"Extracted hyperparameters: hidden_size={hidden_size}, num_layers={num_layers}")
