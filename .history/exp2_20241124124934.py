...
def run_experiments(train_loader, test_loader, vocab, device, config, exp_dir):
    ...
    Save all results (metrics, checkpoints, sample generations) to the experiment directory.
    """
    results = {}
    ...
    for name, model in models.items():
        ...
        # Generate sample text and save
        sample_generations = {}
        for seed in config['sample_seeds']:
            generated_text = generate_text(model, vocab, config['line_tokenizer'], seed, max_length=30)
            sample_generations[seed] = generated_text
            
        # Save sample generations
        sample_path = os.path.join(exp_dir, f"{name}_generations.txt")
        with open(sample_path, "w", encoding="utf-8") as f:
            for seed, text in sample_generations.items():
                f.write(f"Seed: {seed}\nGenerated: {text}\n{'-' * 50}\n")
        print(f"Sample generations for {name} saved to {sample_path}")
