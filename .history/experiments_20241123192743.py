...
from models import GRUWord2Vec, LSTMOneHot, GRUOneHot, LSTMWord2Vec, LSTM, GRU
...
from config import hidden_size, num_layers, nepochs, learning_rate
...
def experiment_1(train_loader, test_loader, vocab, device):
    ...
    # Initialize models
    lstm_model = LSTM(len(vocab), hidden_size, num_layers).to(device)
    gru_model = GRU(len(vocab), hidden_size, num_layers).to(device)
    ...
    print(f"LSTM Training Time: {lstm_training_time:.2f}s")
...
def experiment_2(train_loader, test_loader, vocab, tokenized_corpus, device):
    ...
    model = GRUWord2Vec(len(vocab), hidden_size, num_layers, w2v_model, vocab).to(device)
