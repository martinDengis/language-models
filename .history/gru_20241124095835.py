...
from lstm import create_experiment_dir, calculate_perplexity, generate_text, save_training_plots, hyperparams, corpus_path, vocab, train_loader, test_loader, tokenizer, sample_seeds
...
# Download the results from the LSTM model
lstm_results_path = os.path.join(exp_dir, "lstm_results.json")
with open(lstm_results_path, "r", encoding='utf-8') as f:
    lstm_results = json.load(f)
    
# Extract the necessary values from the LSTM results
lstm_train_losses = lstm_results["train_losses"]
lstm_test_losses = lstm_results["test_losses"]
lstm_perplexities = lstm_results["perplexities"]
lstm_training_time = lstm_results["training_time"]
lstm_params = lstm_results["total_params"]
lstm_total_params = lstm_params["total_parameters"]
