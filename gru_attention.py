...
class AttentiveGRU(nn.Module):
    def __init__(self, num_emb, output_size, num_layers, hidden_size, dropout=0.2):
        ...
        # Output transformation layer after attention
...
def generate_text(model, vocab, tokenizer, seed_text, max_length=20):
    ...
    # Initialize hidden states using model's init_hidden method
    hidden = model.init_hidden(1, device)  # batch size of 1 for generation
    ...
    with torch.no_grad():
        for _ in range(max_length):
            ...
            # Get model prediction with attention
            output, hidden, _ = model(input_sequence, hidden)  # Now handles attention output
...
def analyze_attention_weights(model, input_sequence, vocab, tokenizer):
    model.eval()
    with torch.no_grad():
        # Prepare input
        tokens = tokenizer(input_sequence)
        input_tensor = torch.tensor([vocab[token] for token in tokens], dtype=torch.long).unsqueeze(0).to(device)
...
# Initialize Attentive GRU Model
print("Initializing Attentive GRU model...")
gru_model = AttentiveGRU(num_emb=len(vocab),
...
# Print model summary
print("\nAttentive GRU Model Architecture:")
print(gru_model)

# Calculate total parameters
gru_total_params = sum(p.numel() for p in gru_model.parameters())
print(f"\nTotal Attentive GRU parameters: {gru_total_params:,}")
...
# GRU Training Loop
print("\nStarting Attentive GRU Training...")
# Initialize the progress bar
gru_epoch_bar = trange(nepochs, desc="Attentive GRU Training Progress")
...
for epoch in gru_epoch_bar:
    ...
    with torch.no_grad():
        for sequences, targets in tqdm(test_loader, desc="Validation", leave=False):
            ...
            # Forward pass with attention for validation
            output, hidden, _ = gru_model(sequences, hidden)
    ...
    # Generate sample text and analyze attention every few epochs
    if (epoch + 1) % 2 == 0:
        sample_text = generate_text(gru_model, vocab, tokenizer, "Le président", max_length=20)
        print(f"\nAttentive GRU Sample text generation: {sample_text}\n")
        
        # Analyze attention weights for the sample
        attention_fig = analyze_attention_weights(gru_model, "Le président", vocab, tokenizer)
        attention_fig.savefig(os.path.join(exp_dir, f'attention_epoch_{epoch+1}.png'))
        plt.close()
