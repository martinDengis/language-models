...
class AttentiveLSTM(nn.Module):
    def __init__(self, num_emb, output_size, num_layers=1, hidden_size=128, dropout=0.2):
    ...
    def forward(self, x, hidden, memory):
        ...
        lstm_out, (hidden, memory) = self.lstm(embedding, (hidden, memory))
        
        all_attention_weights = []
        outputs = []
        
        for t in range(seq_length):
            curr_hidden = lstm_out[:, t, :]
            
            attention_weights = []
            for s in range(seq_length):
                score = self.attention_score(lstm_out[:, s, :])
            ...
            combined = torch.cat([curr_hidden, context], dim=1)
        ...
        return outputs, hidden, memory, all_attention_weights
...
def generate_text(model, vocab, tokenizer, seed_text, max_length=20):
    ...
    with torch.no_grad():
        for _ in range(max_length):
            ...
            # Get model prediction (ignore attention weights)
            output, hidden, memory, _ = model(input_sequence, hidden, memory)
...
def analyze_attention_weights(model, input_sequence, vocab, tokenizer):
    ...
    with torch.no_grad():
        ...
        _, _, _, attention_weights = model(input_tensor, hidden, memory)
        
        # Average attention weights across all positions
        avg_attention = attention_weights.mean(dim=1).squeeze(0).cpu().numpy()
        
        # Create visualization
        plt.figure(figsize=(10, 6))
        plt.imshow(avg_attention.reshape(1, -1), aspect='auto', cmap='Blues')
        plt.colorbar()
        plt.xticks(range(len(tokens)), tokens, rotation=45)
        plt.yticks([0], ['Average Attention'])
...
lstm_model = AttentiveLSTM(num_emb=len(vocab),
...
for epoch in lstm_epoch_bar:
    ...
    for sequences, targets in batch_bar:
        ...
        # Initialize hidden states using the model's method
        hidden, memory = lstm_model.init_hidden(bs, device)

        # Forward pass
        output, hidden, memory, _ = lstm_model(sequences, hidden, memory)
    ...
    if (epoch + 1) % 2 == 0:
        attention_fig = analyze_attention_weights(lstm_model, "Le président", vocab, tokenizer)
